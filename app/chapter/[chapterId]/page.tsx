  ...
  useEffect(() => {
    ...
    const loadChapter = async () => {
      try {
        ...
        const mangaUnlocked = localStorage.getItem(
  `manga-unlocked-${data.chapter.manga.id}`
);

if (mangaUnlocked === "true") {
  data.chapter.isLocked = false;
  data.chapter.manga.isLocked = false;
}
...
setChapter((current) =>
  ...
  >
    ...
    {image.fileName !== "credit-re.jpg" && (
      <div
        className="absolute bottom-4 right-4 z-10 font-['SJ_Awesome'] text-[20px] font-normal leading-none text-black"
      >
        Hãy xem tại yoruteam.com để ủng hộ nhóm dịch
      </div>
    )}
  </div>
))}
