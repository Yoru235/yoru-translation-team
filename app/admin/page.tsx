...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<"manga" | "chapter" | "group">(
    "manga",
  );
  ...
  const handleLogout = async () => {
    try {
      ...
      try {
        ...
        console.error("LOGOUT RESPONSE KHÔNG PHẢI JSON:", responseText);
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.error || "Không thể đăng xuất.");
      }
    ...
    const loadMangas = async () => {
      try {
        ...
        if (data.mangas.length > 0) {
          setSelectedManga(data.mangas[0].id);
        } else {
          setSelectedManga("");
        }
      } catch (error) {
        ...
        setMessage("❌ Không thể tải danh sách truyện.");
      } finally {
  ...
  const handleCoverChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setMessage(`🖼️ Đã chọn ảnh bìa: ${file.name}`);
  };
  ...
  const handleCreditChange = (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    ...
    setCreditPreview(previewUrl);

    setMessage(`🎨 Đã chọn ảnh credit/re: ${file.name}`);
  };
  ...
  const selectedGenreNames = useMemo(() => {
    return genres
      .filter((genre) => mangaForm.genres.includes(genre.id))
  ...
  const processImageFiles = async (files: File[]) => {
    ...
    if (imageFiles.length === 0) {
      setMessage("❌ Không tìm thấy file ảnh hợp lệ.");
      return;
    }
    ...
    setMessage(`⏳ Đang chuẩn bị ${imageFiles.length} ảnh...`);
    ...
    try {
      ...
      const previewImages: ZipImage[] = sortedFiles.map((file, index) => ({
        name: file.name,
        size: file.size,
      ...
      setZipImages(previewImages);

      setMessage(`✅ Đã chọn thành công ${previewImages.length} ảnh.`);
    } finally {
  ...
  const processZipFile = async (file: File) => {
    if (!file.name.toLowerCase().endsWith(".zip")) {
      setMessage("❌ File này không phải ZIP.");
      return;
    }
    ...
    setMessage(`📦 Đang đọc và giải nén ${file.name}...`);
    ...
    try {
      ...
      if (imageEntries.length === 0) {
        setMessage("❌ Không tìm thấy ảnh JPG, PNG hoặc WEBP bên trong ZIP.");
      ...
      setMessage(`✅ Đã giải nén thành công ${previewImages.length} ảnh.`);
    } catch (error) {
      ...
      setMessage("❌ Không thể đọc file ZIP. Vui lòng kiểm tra lại file.");
    } finally {
      setIsProcessingZip(false);
    }
  };
...
===================================== */
  ...
  const handleCreateManga = async () => {
    ...
    try {
      ...
      coverFormData.append("file", mangaForm.cover);
      ...
      if (!coverResponse.ok || !coverData.success || !coverData.imageUrl) {
        throw new Error(coverData.error || "Không thể upload ảnh bìa.");
      }

      const coverUrl = coverData.imageUrl;
      ...
      if (mangaForm.credit) {
        setMessage("⏳ Đang upload ảnh credit...");

        const creditFormData = new FormData();

        creditFormData.append("file", mangaForm.credit);

        creditFormData.append("type", "credit");

        const creditResponse = await fetch("/api/upload", {
          method: "POST",
          body: creditFormData,
        });
        ...
        if (!creditResponse.ok || !creditData.success || !creditData.imageUrl) {
          throw new Error(creditData.error || "Không thể upload ảnh credit.");
        }

        creditUrl = creditData.imageUrl;
      }
      ...
      const response = await fetch("/api/admin/manga", {
        ...
        body: JSON.stringify({
          ...
          originalTitle: mangaForm.originalTitle.trim(),

          author: mangaForm.author.trim(),
          translationGroupId: mangaForm.translationGroupId || null,

          releaseDate: mangaForm.releaseDate || null,
          ...
          ageRestricted: mangaForm.ageRestricted,
      ...
      setMessage(`✅ Tạo truyện thành công: "${data.manga.title}"`);

      if (data.manga?.id && data.manga?.title) {
        ...
        setMangaList((prev) => [
          newManga,
          ...prev.filter((item) => item.id !== newManga.id),
        ]);

        setSelectedManga(newManga.id);
  ...
  const handleGroupAvatarChange = (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    ...
    try {
      ...
      if (groupAvatarFile) {
        ...
        if (!uploadResponse.ok || !uploadData.success) {
          throw new Error(uploadData.error || "Không thể upload avatar nhóm.");
        }

        finalAvatarUrl =
          uploadData.imageUrl || uploadData.images?.[0]?.imageUrl || null;

        if (!finalAvatarUrl) {
          throw new Error("API không trả về URL avatar.");
        }
      }
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.error || "Không thể tạo nhóm dịch.");
      }

      setTranslationGroups((current) => [data.group, ...current]);
      ...
      setMessage(
        error instanceof Error ? error.message : "Không thể tạo nhóm dịch.",
      );
    } finally {
      setIsCreatingGroup(false);
    }
  };
  const handleDeleteTranslationGroup = async (groupId: string) => {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.error || "Không thể xóa nhóm dịch.");
      }
      ...
      setMessage(
        error instanceof Error ? error.message : "Không thể xóa nhóm dịch.",
      );
    }
  };
  const handleEditTranslationGroup = async () => {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.error || "Không thể sửa nhóm dịch.");
      }

      setTranslationGroups((current) =>
        current.map((group) =>
          group.id === editingGroupId ? data.group : group,
        ),
      );
      ...
      setMessage(
        error instanceof Error ? error.message : "Không thể sửa nhóm dịch.",
      );
    } finally {
      setIsSavingGroup(false);
    }
  };
  const startEditTranslationGroup = (group: TranslationGroupItem) => {
  ...
  const handleUploadChapter = async () => {
    if (!selectedManga) {
      setMessage("❌ Vui lòng chọn truyện.");
      return;
    }

    if (!chapter.trim()) {
      setMessage("❌ Vui lòng nhập số chapter.");
      return;
    }
    ...
    if (!Number.isFinite(chapterNumber) || chapterNumber < 0) {
      setMessage("❌ Số chapter không hợp lệ.");
      return;
    }

    if (!Number.isInteger(chapterNumber)) {
      setMessage("❌ Số chapter phải là số nguyên.");
      return;
    }
    ...
    if (volume.trim()) {
      ...
      if (!Number.isFinite(volumeNumber) || volumeNumber < 1) {
        setMessage("❌ Volume không hợp lệ.");
        return;
      }

      if (!Number.isInteger(volumeNumber)) {
        setMessage("❌ Volume phải là số nguyên.");
        return;
      }
    }
    ...
    if (
      ...
      setMessage("❌ Novel cần có nội dung hoặc ít nhất một ảnh.");
      return;
    }
    ...
    if (chapterType !== "Novel" && zipImages.length === 0) {
      setMessage("❌ Chưa có ảnh để upload.");
      return;
    }

    const selectedMangaData = mangaList.find(
      (manga) => manga.id === selectedManga,
    );

    if (!selectedMangaData) {
      setMessage("❌ Không tìm thấy truyện đã chọn.");
      return;
    }

    try {
      setMessage(`⏳ Đang upload ${zipImages.length} ảnh...`);
      ...
      for (let index = 0; index < zipImages.length; index++) {
        const image = zipImages[index];

        setMessage(`⏳ Đang upload ảnh ${index + 1}/${zipImages.length}...`);
        ...
        if (image.file) {
          ...
          if (!imageResponse.ok) {
            throw new Error(`Không thể đọc ảnh ${image.name}.`);
          }

          const blob = await imageResponse.blob();

          file = new File([blob], image.name.split("/").pop() || image.name, {
            type: blob.type || "image/jpeg",
          });
        }
        ...
        if (!uploadedImage?.imageUrl) {
          throw new Error(`API không trả về ảnh sau khi upload ${file.name}.`);
        }
      ...
      setMessage("⏳ Đang tạo chapter trong database...");
      ...
      try {
        ...
        console.error("CREATE CHAPTER RESPONSE:", responseText);
      ...
      if (!response.ok) {
        console.error("CREATE CHAPTER ERROR:", response.status, responseText);

        throw new Error(
          data?.error || `Không thể tạo chapter. HTTP ${response.status}.`,
        );
      }

      if (!data?.success) {
        console.error("CREATE CHAPTER INVALID RESPONSE:", data);

        throw new Error(
          data?.error || "API không trả về kết quả tạo chapter hợp lệ.",
        );
      }
  ...
  return (
    <main className="min-h-screen bg-[#09060d] text-white">
          ...
          <div className="rounded-3xl border border-purple-800 bg-[#0f0a14] p-8 shadow-lg">
            ...
            <div className="mb-8">
              ...
              <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
                ...
                <div className="text-sm text-purple-400">
                  <p>Chọn ảnh bìa cho truyện.</p>

                  <p className="mt-2">JPG, PNG hoặc WEBP.</p>
                </div>
              </div>
            </div>
              ...
              {!isLoadingTranslationGroups &&
                translationGroups.length === 0 && (
                  <p className="mt-2 text-xs text-purple-500">
                    Chưa có nhóm dịch nào. Hãy tạo nhóm ở mục "Quản lý nhóm
                    dịch".
                  </p>
                )}
            </div>
            ...
            <div className="mb-7">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {["Manga", "Manhwa", "Manhua", "Novel"].map((type) => (
                  <button
                    type="button"
                    key={type}
                    onClick={() =>
              ...
              <div className="flex flex-wrap gap-3">
                {genres.map((genre) => (
                  <button
                    ...
                    className={
                      mangaForm.genres.includes(genre.id)
            ...
            <div className="mb-8 rounded-2xl border border-red-900 bg-[#1d0b10] p-5">
              <label className="flex cursor-pointer items-start gap-3">
                <input
                  type="checkbox"
                  checked={mangaForm.ageRestricted}
                  onChange={(event) =>
                    setMangaForm((prev) => ({
                      ...prev,
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm leading-6 text-red-300">
                    Truyện được đánh dấu sẽ thuộc khu vực nội dung giới hạn độ
                    tuổi và chỉ hiển thị sau khi người đọc xác nhận phù hợp độ
                    tuổi.
                  </p>
                </div>
              </label>
            ...
            <div className="mb-8">
              ...
              <p className="mb-4 text-sm text-purple-400">
                Ảnh này sẽ được tự động thêm vào cuối chapter.
              </p>
        ...
        {activeTab === "chapter" && (
          <div className="rounded-3xl border border-purple-800 bg-[#0f0a14] p-8 shadow-lg">
            ...
            <div className="mb-5">
              ...
              <select
                value={selectedManga}
                onChange={(event) => setSelectedManga(event.target.value)}
              ...
              {!isLoadingMangas && mangaList.length > 0 && (
                <p className="mt-2 text-xs text-purple-500">
                  Đang có{" "}
              ...
              <div className="mb-6 rounded-2xl border border-purple-800 bg-[#120c18] p-5">
                <label className="flex cursor-pointer items-center gap-3 font-bold text-purple-200">
                  <input
                    ...
                    onChange={(event) =>
                      setIsWatermarkEnabled(event.target.checked)
                    }
                    className="h-5 w-5"
                  />
              ...
              >
                <label className="block cursor-pointer">
                  ...
                  <p className="mt-4 text-xs text-purple-500">
                    ZIP sẽ tự động giải nén; ảnh thường sẽ được sắp xếp theo tên
                    file.
                  </p>
                </label>
              </div>
            ...
            {zipFile && !isProcessingZip && (
              <div className="mb-6 rounded-2xl border border-pink-900 bg-[#24101f] p-5">
                ...
                <p className="mt-1 text-xs text-purple-500">
                  Dung lượng: {(zipFile.size / 1024 / 1024).toFixed(2)} MB
                </p>
              </div>
            )}
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                {zipImages.length > 0 && (
                  <div className="rounded-full bg-[#24152f] px-4 py-2 text-sm font-bold text-purple-200 shadow-sm">
                    {zipImages.length} ảnh
                  </div>
                )}
              </div>

              {zipImages.length === 0 && !isProcessingZip && (
                <div className="mt-5 rounded-2xl bg-[#0f0a14] p-8 text-center">
                  ...
                  <p className="mt-2 text-sm text-purple-400">
                    Hãy chọn hoặc kéo một file ZIP hoặc nhiều ảnh vào khu vực
                    phía trên.
                  </p>
                </div>
              )}
            ...
            {zipImages.length > 0 && (
              <div className="mb-6 rounded-2xl border border-green-900 bg-[#0b1a12] p-5">
                ...
                <p className="mt-2 text-sm leading-6 text-green-300">
                  Hệ thống sẽ xử lý ảnh theo thứ tự hiển thị bên trên:
                  <strong> 1 → 2 → 3 → 4 → 5 → ...</strong>
                </p>

                <p className="mt-2 text-xs text-green-500">
                  Bạn có thể kiểm tra toàn bộ ảnh trước khi bấm Upload Chapter.
                </p>
              </div>
            )}
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-purple-800 bg-[#0f0a14] p-4 hover:bg-[#24152f]">
                  <input
                    type="checkbox"
                    checked={isH}
                    onChange={(event) => setIsH(event.target.checked)}
                    className="h-5 w-5 accent-purple-700"
                  />

                  <div>
                    <p className="font-bold text-purple-200">H</p>
                ...
                <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-purple-800 bg-[#0f0a14] p-4 hover:bg-[#24152f]">
                  ...
                  <div>
                    <p className="font-bold text-purple-200">END</p>
            ...
            <div className="mb-6 rounded-2xl border border-fuchsia-900 bg-[#21101f] p-5">
              ...
              <p className="mt-2 text-sm leading-6 text-purple-400">
                Nếu bộ truyện đã có ảnh credit/re, hệ thống sẽ đưa ảnh đó xuống
                cuối chapter sau toàn bộ ảnh truyện.
              </p>
            </div>
        ...
        {activeTab === "group" && (
          <div className="rounded-3xl border border-purple-800 bg-[#0f0a14] p-8 shadow-lg">
            ...
            <div className="grid gap-5">
              ...
              >
                {isCreatingGroup ? "⏳ Đang tạo..." : "➕ Tạo nhóm dịch"}
              </button>
            </div>
      ...
      <footer className="mt-10 bg-gradient-to-r from-[#4b176d] via-[#812681] to-[#c9328d] px-6 py-8 text-center text-white">
        ...
        <p className="mt-1 text-xs text-purple-200">Khu vực quản trị</p>
      </footer>
    </main>
  );
}
