...
export default function AdminMangaPage() {
  ...
  const loadMangas = async (currentPage = page) => {
    try {
      setIsLoading(true);
      setError("");

      const response = await fetch(
        `/api/admin/manga/paginated?page=${currentPage}&limit=${limit}`,
        {
          method: "GET",
          cache: "no-store",
        },
      ...
      // Lưu thông tin thống kê chuẩn từ backend
      if (data.stats) {
        setStats(data.stats);
      }
  ...
  // Tự động gọi lại loadMangas khi `page` thay đổi
  useEffect(() => {
    void loadMangas(page);
  }, [page]);

  // Reset về trang 1 khi người dùng tìm kiếm hoặc lọc trạng thái
  ...
  const handleDeleteManga = async (mangaId: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/manga?id=${mangaId}`, {
        method: "DELETE",
      });

      const text = await response.text();

      console.log("DELETE MANGA STATUS:", response.status);

      console.log("DELETE MANGA RESPONSE:", text);
      ...
      if (text.trim()) {
        try {
          ...
          throw new Error("API xóa truyện trả về dữ liệu không hợp lệ.");
        }
      }

      if (!response.ok || !data.success) {
        throw new Error(data.error || "Không thể xóa truyện.");
      }

      // Xóa truyện khỏi danh sách hiện tại
      setMangas((currentMangas) =>
        currentMangas.filter((manga) => manga.id !== mangaId),
      );
    } catch (err) {
  ...
  const filteredMangas = mangas.filter((manga) => {
    ...
    const matchesStatus =
      statusFilter === "all" || manga.status.toLowerCase() === statusFilter;
    ...
    <main className="min-h-screen bg-[#080808] text-white">
      ...
      <header className="sticky top-0 z-50 border-b border-purple-900 bg-black/95 backdrop-blur">
        <div className="mx-auto flex min-h-20 max-w-7xl items-center justify-between gap-4 px-6">
          <div>
            ...
            <h1 className="text-2xl font-extrabold">Quản lý truyện</h1>
          </div>
      ...
      <section className="border-b border-gray-900 bg-[#0b0b0b]">
        <div className="mx-auto max-w-7xl px-6 py-8">
          <h2 className="text-3xl font-extrabold">Danh sách truyện</h2>
        ...
        <div className="mx-auto max-w-7xl px-6 py-8">
          ...
          <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div className="rounded-2xl border border-purple-900 bg-[#111111] p-5">
              <p className="text-sm text-gray-500">Tổng số truyện</p>
            ...
            <div className="rounded-2xl border border-pink-900 bg-[#111111] p-5">
              ...
              <p className="mt-2 text-3xl font-extrabold text-pink-400">
                {/* {
                  mangas.filter(
                    (manga) => manga.status.toLowerCase() === "ongoing",
                  ).length
                } */}
                {stats.ongoing}
              </p>
            ...
            <div className="rounded-2xl border border-fuchsia-900 bg-[#111111] p-5">
              ...
              <p className="mt-2 text-3xl font-extrabold text-fuchsia-400">
                {/* {
                  mangas.filter(
                    (manga) => manga.status.toLowerCase() === "completed",
                  ).length
                } */}
                {stats.completed}
              </p>
              ...
              {totalPages > 1 && (
                <div className="mt-8 flex items-center justify-center gap-2">
                  <button
                    ...
                    className="rounded-xl border border-gray-700 bg-[#111111] px-4 py-2 text-sm font-bold text-gray-300 transition hover:border-purple-600 hover:text-purple-300 disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    ← Trước
                  </button>
                      ...
                      (p) => (
                        <button
                          ...
                          className={`h-10 w-10 rounded-xl text-sm font-bold transition ${
                            page === p
                              ? "bg-purple-700 text-white"
                              : "border border-gray-800 bg-[#111111] text-gray-400 hover:border-purple-600 hover:text-purple-300"
                          }`}
                        >
                          {p}
                        </button>
