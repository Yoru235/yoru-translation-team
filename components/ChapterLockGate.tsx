...
type Props = {
  ...
  isMangaLocked: boolean;
  passwordHint: string | null;
};

export default function ChapterLockGate({
  mangaId,
  chapterId,
  mangaTitle,
  chapterNumber,
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Không thể mở khóa Chapter."
      );
    } finally {
        ...
        <div className="w-full max-w-md rounded-2xl border border-yellow-900 bg-[#111111] p-8">
          <div className="text-center">
            ...
            <h1 className="mt-4 text-2xl font-bold">
              {isMangaLocked ? "Truyện đang bị khóa" : "Chapter đang bị khóa"}
            </h1>

            <p className="mt-3 text-gray-400">
              {isMangaLocked
                ? `Nhập mật khẩu để đọc truyện "${mangaTitle}"`
                : `Nhập mật khẩu để đọc Chapter ${chapterNumber} của "${mangaTitle}"`}
            </p>
